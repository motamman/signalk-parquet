...
- **Startup walked the whole store four times on the server's main thread** — on a store with thousands of vessel contexts (a shore station hearing AIS: ~500,000 directories, ~680,000 parquet files) every plugin start held the Signal K server unresponsive for about three minutes: HTTP timed out, websockets took up to fifteen seconds to open. Measured on a clean restart: the startup cloud sync's seven `tier=*/**/year=Y/day=D/*.parquet` globs (one per day of the lookback, each a walk of the entire tree) took ~116 s; the empty-parquet quarantine sweep 30 s; the stranded compaction temp-file cleanup 18 s; compaction trash recovery 4 s. PR #88 (0.7.41-beta.3) had removed the `**/*.parquet` glob from the quarantine sweep and made its file stats incremental, but the walk still descended every directory, and its changelog claim that it "no longer blocks the event loop" held only for the stat pass; the same `**` pattern was already in the cloud sync and was added again with per-year compaction. Fixed at the root: a new layout-aware lister (`utils/hive-walk.ts`) walks the known tier/context/path/year/day levels, prunes by tier, year and day, never uses `**`, and yields to the event loop every hundred readdirs; the startup and daily cloud sync listings, the compaction temp cleanup and trash recovery all use it (the seven-day listing is exact, as before). The three crash-recovery sweeps now run in a short-lived forked worker (`startup-sweep-worker.ts`, the daily aggregation's arrangement), so whatever they cost on a large store never touches the main thread; they are still awaited, so they finish before DuckDB opens and before any data subscription lands. A worker that cannot be forked runs the sweeps in-process; one that dies or times out is logged and start continues. One `[StartupSweep]` debug line reports the counts and duration.
- **Playback spun the server's main thread once it reached the present** — on catching up with live data the replay loop decided whether to wait by comparing the window's end against a fresh clock reading with a one-millisecond tolerance. Reading a window runs one synchronous SQLite query per buffered path, so the comparison always failed on a buffer of any size, the empty window was taken for silence to skip, and the loop refetched at once without ever yielding: 100% CPU on the main thread, no HTTP or websocket served, until the client happened to disconnect. Found on a test server the first time a session ran past the present. `fetch` now reports whether its window reached the present, and the loop enforces that every turn either advances the cursor or sleeps, logging a window that does neither instead of refetching. The test drives a session past the present against a buffer whose reads take a few milliseconds and counts the reads.
...
- **v1 history playback provider** — the plugin now registers with the server's `registerHistoryProvider`, so `/signalk/v1/playback?startTime=…&playbackRate=…` (Freeboard-SK's History Playback) replays the store. A new `PlaybackProvider` reads playback time in 60-second windows from the raw parquet tier and the not-yet-exported SQLite buffer, regroups rows into one delta per instant, vessel and source (`$source` from the recorded source label; object paths rebuilt from their `value_*` component columns or stored JSON; parquet scalars keep their stored type, buffered scalars are parsed back from text), and paces them by `playbackRate`. Each vessel's last known `identity` row is unfolded and sent ahead of its first delta, so plotters label targets at once. Silence is skipped by jumping to the next recorded instant; on reaching the present the buffer is polled once a second so playback runs on into live data; `subscribe=self` (the server's default) limits reads to the own vessel. Per day of playback one metadata-only index of that day's files (time span and value column type from `parquet_schema`/`parquet_metadata`, no rows read) decides which files each window opens; a window over 50,000 rows is halved and re-read. Nothing runs at plugin start. New `SQLiteBuffer` reads for playback (`getRowsForPlayback`, `hasRowsSince`, `getNextRowTime`, `getLatestObjectRowAt`); the buffer read caps rows across all paths together and returns the earliest of the window, merging each path's timestamp-ordered rows and narrowing the next path's query to the result's last timestamp, so a path registered late still contributes rows earlier than one registered first and memory stays bounded by the result plus one path's read.
- **`context` query extension on v1 playback** — `/signalk/v1/playback?…&subscribe=all&context=367390130,self` replays only the named vessels (full contexts, bare MMSIs or `self`, comma-separated). The spec offers only `subscribe=self` or `all`, and the server closes a playback connection that sends a subscribe message, so there was no way to follow one other vessel. Read from the connection's query string, which the server passes to the provider untouched; every read (parquet globs, day index, buffer queries, identity announcements) narrows to the named vessels. Documented as an extension; the same capability is proposed upstream as a server feature.
- **Vessel identity recorded as one `identity` object path per vessel** — name, MMSI, AIS ship type (`aisShipTypeId`, `aisShipTypeName`), `lengthOverall`, `beam`, `callsignVhf` and `aisClass`, folded from the root bus and the `design.*`, `communication.callsignVhf` and `sensors.ais.class` deltas by a new `VesselIdentityService` that runs independently of the path configuration (setting **Record Vessel Identity**, default on). Written for every vessel the server hears, once when first seen (seeded from the in-memory model at start, so the server's own defaults and AIS targets heard before a restart are not missed) and again only when a component changes; the last-written state is persisted to `identity-state.json` so a restart rewrites nothing. Folding seven attributes into one object path keeps the hive layout's one-file-per-path-per-vessel-per-day cost to one file per vessel. The path is retention-exempt and never aggregated, which also stops the tiers filling with meaningless rollups of a configured `name` path. Readable through the History API as an object path; groundwork for playback labels and startup restore of AIS targets.
...
- **Raw SQL and analysis queries failed outright when the spatial extension could not be fetched** — the main DuckDB pool already treats `INSTALL spatial` as best-effort, but the sandboxed instance used by `/api/query` and model-driven analysis threw when the extension was neither cached nor downloadable, so on a server first started offline every sandboxed query failed until a restart with connectivity. The sandbox now degrades the same way the main pool does (spatial functions fail individually, with a warning), and it is created with the main pool's extension directory so a cached extension is found. This was also the one failure in the Signal K plugin registry's test run, which executes the suite with no network; the published tarball now includes `CHANGELOG.md` for the registry's changelog check.
