...
export function cancelValidation() {
    ...
    if (currentValidationJobId) {
        ...
        fetch(`${getPluginPath()}/api/validate-schemas/cancel/${currentValidationJobId}`, {
            method: 'POST'
        }).then(response => {
            if (!response.ok) {
...
export function cancelRepair() {
    ...
    if (currentRepairJobId) {
                fetch(`${getPluginPath()}/api/repair-schemas/cancel/${currentRepairJobId}`, {
            method: 'POST'
        }).catch(error => {
...
async function pollValidationProgress(jobId, timerElement, startTime, abortSignal) {
    ...
    while (polling) {
        try {
            ...
            const response = await fetch(`${getPluginPath()}/api/validate-schemas/progress/${jobId}`, {
                signal: abortSignal,
                cache: 'no-store'
            });
...
async function pollRepairProgress(jobId, timerElement, startTime, abortSignal) {
    ...
    while (polling) {
        try {
            ...
            const response = await fetch(`${getPluginPath()}/api/repair-schemas/progress/${jobId}`, {
                signal: abortSignal,
                cache: 'no-store'
            });
...
export async function runDataValidation() {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            ...
            try {
                if (currentValidationJobId) {
                    const progressResponse = await fetch(`${getPluginPath()}/api/validate-schemas/progress/${currentValidationJobId}`, {
                        cache: 'no-store'
                    });
