...
export async function loadHomePort() {
  try {
    ...
    if (data.success && data.latitude !== null && data.longitude !== null) {
      ...
      showHomePortStatus(
        `✅ Loaded: ${data.latitude.toFixed(6)}, ${data.longitude.toFixed(6)}`,
        'success'
      );
...
export async function saveHomePort() {
  ...
  try {
    ...
    if (data.success) {
      showHomePortStatus(
        `✅ Home port saved: ${latitude.toFixed(6)}, ${longitude.toFixed(6)}`,
        'success'
      );
    } else {
...
export async function setCurrentLocationAsHomePort() {
  try {
    ...
    if (data.success) {
      ...
      showHomePortStatus(
        `✅ Current position loaded: ${data.latitude.toFixed(6)}, ${data.longitude.toFixed(6)}`,
        'success'
      );
    } else {
