...
export async function testClaudeConnection() {
    ...
    try {
const response = await fetch(`${getPluginPath()}/api/analyze/test-connection`, {
    method: 'POST',
...
console.log('Refreshing vessel info from SignalK...');
        const response = await fetch(`${getPluginPath()}/api/vessel-context/refresh`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' }
});
...
console.log('Generating Claude context preview...');
        const response = await fetch(`${getPluginPath()}/api/vessel-context/claude-preview`);
...
function populateTemplateDropdown(templates) {
    ...
    if (!select) {
        console.warn('analysisTemplate select element not found; skipping dropdown population.');
        return;
    }
    ...
    select.onchange = event => {
        ...
        if (defaultPath) {
            ...
            alert('This template does not define a default data path. Select paths manually, adjust the prompt if needed, then run a custom analysis.');
        }
    };
}
...
function populateTemplateCards(templates) {
    ...
    if (!Array.isArray(templates) || templates.length === 0) {
        container.innerHTML = '<div style="color: #666; font-style: italic;">No templates available yet.</div>';
        return;
    }
...
export async function openAnalysisHistoryModal() {
    ...
    try {
const response = await fetch(`${getPluginPath()}/api/analyze/history?limit=20`);
if (!response.ok) {
    throw new Error(`HTTP ${response.status}: ${response.statusText}`);
}
...
const historyResponse = await fetch(`${getPluginPath()}/api/analyze/history`);
...
export async function deleteAnalysis(analysisId, isFullView = false) {
    try {
        ...
        const response = await fetch(`${getPluginPath()}/api/analyze/history/${analysisId}`, {
            method: 'DELETE'
        });
...
export async function analyzeDataPath(signalkPath, directory) {
