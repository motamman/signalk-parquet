 ...
 *
 * This guard supplies the missing half: every statement must be a read-only
 * query. Two rules, because the dangerous constructs arrive in two shapes.
 *
 ...
 *
 * 2. Function-name rejection, for the table functions that reach outside an
 *    otherwise valid SELECT: `query()` and `query_table()` execute a nested SQL
 *    string that rule 1 cannot see into, `read_text`/`read_blob`/`glob` read
 *    raw bytes and directory listings, and the `sqlite_*` family opens a SQLite
 *    database directly.
 *
 ...
 * form, so masking identifiers before this check lets the quoted spelling
 * through.
 *
 ...
 * becomes a false positive. That trade is deliberate — the alternative lets
 * the quoted spelling through.
 */
