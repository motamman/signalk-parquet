...
import { DataRecord, ParquetWriterOptions, SignalKApp, ParquetSchema, ParquetField, FileFormat } from './types';
...
export class ParquetWriter {
  ...
  async writeParquet(filepath: string, records: DataRecord[]): Promise<string> {
    try {
      ...
      this.app?.debug(`Attempting to write ${records.length} records to Parquet`);
      ...
      this.app?.debug(`Creating Parquet schema with ${Object.keys(schemaFields).length} fields:`, Object.keys(schemaFields));
      ...
      this.app?.debug(`✅ Successfully wrote ${records.length} records to Parquet: ${filepath}`);
      return filepath;
      ...
      this.app?.debug(`💾 Saving failed Parquet data as JSON to: ${failedPath}`);
      this.app?.debug('⚠️  This data will need manual conversion to maintain DuckDB schema consistency');
  ...
  createParquetSchema(records: DataRecord[]): any {
    ...
    columns.forEach(colName => {
      const values = records.map(r => (r as any)[colName]).filter(v => v !== null && v !== undefined);
          ...
          if (ext === '.json') {
            ...
            if (parquet) {
              try {
                ...
                this.app?.debug(`Failed to read Parquet file ${sourceFile}:`, (parquetError as Error).message);
              }
            }
          } else if (ext === '.csv') {
  ...
  async consolidateDaily(dataDir: string, date: Date, filenamePrefix: string = 'signalk_data'): Promise<number> {
    try {
      ...
      for (const entry of consolidatedFiles) {
        ...
        this.app?.debug(`Consolidated ${entry.sources.length} files into ${entry.target} (${recordCount} records)`);
      ...
      throw new Error(`Failed to consolidate daily files: ${(error as Error).message}`);
    }
  }
}